...
@ViewController("FXMLImportWizardView3.fxml")
public class ImportWizardView3Controller {
    private static final Logger logger = LoggerFactory.getLogger(ImportWizardView3Controller.class);
    ...
    public void init() throws FlowException {
        logger.debug("ImportWizardView3Controller.initialize()");
        ...
        Task importTask = new Task<Void>() {
            ...
            protected Void call() {
                ...
                if (!participantDAO.listParticipants().isEmpty()) {
                    ...
                    logger.debug("Import: dup property is " + model.duplicateHandlingProperty().getValue());
               ...
               try {
                    ...
                    logger.debug("ExistingMap size: " + existingMap.size());
                    ...
                    while (rs.next()) {
                        ...
                        for (int i = 0; i < meta.getColumnCount(); i++) {
                            if (mapping.get(meta.getColumnLabel(i+1)) != null && !"".equals(rs.getString(i+1))) {
                                ...
                                logger.debug(rs.getString(i+1) + " -> " + key);
                                if (key.equals("WAVE")) {
                        ...
                        if (dupeCheck && existingMap.containsKey(key)){
                            logger.debug("Found existing key");
                            if (mergeDupes) {
                                ...
                                if (attributes.containsKey("bib") && !p.getBib().equals(attributes.get("bib"))) {
                                    if (participantDAO.getParticipantByBib(attributes.get("bib")) != null) {
                                        logger.debug("Duplicate bib found!");
                                ...
                                logger.debug("Duplicate participant found, skipping");
                                updateMessage("Skipping " + p.getFirstName() + " " + p.getLastName() );
                                updateProgress(numAdded,numToAdd);
                                continue;
                            }
                        } else {
                            logger.debug("Did not find an existing person");
                        }
                        ...
                        if(model.getWaveAssignByBib()) {
                            ...
                            logger.debug("Assigning wave by attribute: \"" + pendingWave + "\" / \"" + pendingRace + "\"");
                           ...
                           if (!p.wavesObservableList().isEmpty() ) logger.debug("Now in wave " + p.wavesObservableList().get(0).getWaveName());
                           else logger.debug("Not in any wave/race!!!");
                        } else {
                        ...
                        //logger.debug("Adding " + newPerson.getFirstName() + " " + newPerson.getLastName() );
